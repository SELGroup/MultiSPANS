...
class StandardIndependCScaler(Scaler):
    ...
    def __init__(self, x_train):
        ...
        # self._channel_mean = th.Tensor(self._channel_mean)
        # self._channel_std = th.Tensor(self._channel_std)
            # self.y_channel_dict.append[{'mean':y_train[...,d].mean(),'std':y_train[...,d].std()}]

    def transform(self, data, **kw):
        assert (data.shape[-1]==self.dim), 'Bad channel num for this scalar.'
        return (data - self._channel_mean) / self._channel_std
        # return (data - self._channel_mean.view([...,self.dim])) / self._channel_std.view([...,self.dim])

    def inverse_transform(self, data, **kw):
        if type(data)==th.Tensor:
            _channel_mean = th.from_numpy(self._channel_mean).to(data.device)
            _channel_std = th.from_numpy(self._channel_std).to(data.device)
        ...
        # return (data * self._channel_std.view([...,self.dim])) + self._channel_mean.view([...,self.dim])
