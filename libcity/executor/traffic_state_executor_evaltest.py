...
class TrafficStateExecutorTest(AbstractExecutor):
    ...
    def _build_train_loss(self):
        ...
        self._logger.info('Loss will be calcuated upon {} data.'.format('denormalized' if self.if_inv_loss else 'normalized'))
        ...
        def func(batch,channel_index):
            ...
            if self.if_inv_loss:
                ...
                y_predicted = self._scaler.inverse_transform(y_predicted[..., :self.output_dim],channel_idx=channel_index)
    ...
    def evaluate(self, test_dataloader):
        ...
        self._logger.info('Evaluate upon {} data.'.format('denormalized' if self.if_inv_eval else 'normalized'))
        with torch.no_grad():
            self.model.eval()
            ...
            for batch in test_dataloader:
                ...
                #     y_true = batch['y'][...,self.pred_channel_idx] 
                # else: 
                y_true = batch['y']
                if self.if_inv_eval:
                    y_true = self._scaler.inverse_transform(y_true[..., :self.output_dim])
                    y_pred = self._scaler.inverse_transform(output[..., :self.output_dim],channel_idx=self.pred_channel_idx)
                else:
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader):
        ...
        for epoch_idx in range(self._epoch_num, self.epochs):
            ...
            self._logger.info("validating now!")
            t2 = time.time()
    ...
    def _test_epoch(self, test_dataloader, epoch_idx):
        with torch.no_grad():
            ...
            str({ metric+'@'+str(12):test_result[metric+'@'+str(12)] for metric in self.evaluator.metrics})
            )